...
def get_ai_prediction(lich_su_10_van_str, thong_ke_100_van):
    ...
    # --- PROMPT TỐI THƯỢNG: KẾT HỢP CẢ 2 NGUỒN DỮ LIỆU ---
    prompt = f"""
    ...
    Các phòng: {list(room_mapping.keys())}
    ...
    **1. Dữ liệu dài hạn (Thống kê 100 ván gần nhất):**
    ...
    {json.dumps(thong_ke_100_van, indent=2)}
    ...
    **Nhiệm vụ của bạn:**
    ...
    2.  Dựa vào dữ liệu ngắn hạn, xác định các xu hướng gần đây và **quan trọng nhất là loại trừ phòng vừa có sát thủ ở ván gần nhất**.
    3.  Cân nhắc cả hai yếu tố trên để đưa ra một lựa chọn duy nhất cho phòng bạn tin là **AN TOÀN NHẤT** để cược cho ván tiếp theo. Ưu tiên các phòng an toàn trong dài hạn nhưng không xuất hiện sát thủ trong vài ván gần đây.
    ...
    try:
        ...
        print(Fore.RED + f"❌ Lỗi khi gọi hoặc phân tích phản hồi từ Gemini AI: {e}")
        return None

# --- THU THẬP THÔNG TIN VÀ CÀI ĐẶT ---
clear_screen()
print_colored_ascii_art("PHUOCAN AI v4")
print(Fore.CYAN + "Bot cược Escape Master - Phiên bản phân tích tổng hợp (Dựa trên góp ý của bạn)\n")

try:
    import google.generativeai as genai
except ImportError:
    print(Fore.RED + "Lỗi: Vui lòng cài đặt thư viện 'pip install google-generativeai'"); exit()
...
stop_loss_amount = 0; take_profit_amount = 0
...
api_100_stats = "https://api.escapemaster.net/escape_game/recent_100_issues?asset=BUILD" # Đổi tên cho rõ nghĩa
...
# --- CÁC HÀM LOGIC CHÍNH CỦA GAME ---
def Login():
    global so_du_ban_dau
    print("\n" + Fore.YELLOW + "🔄 Đang đăng nhập vào game...")
    try:
        response = requests.get(url_login, headers=headers); response.raise_for_status()
        data = response.json()
...
def tong_loi_lo():
    # (Code giữ nguyên như v3, không cần thay đổi)
...
def dat_cuoc(room_id, bet_amount):
    ...
    try:
        ...
        if response.status_code == 200 and data.get("code") == 0:
            room_name = reverse_room_mapping.get(room_id, "?")
            print(Fore.BLUE + f"🎯 Đặt cược thành công {int(bet_amount)} BUILD vào phòng '{room_name}'")
        ...
        while tool_running:
            ...
            try:
                ...
                if not (data_10.get("code") == 0 and isinstance(data_10.get("data"), list) and data_10["data"]):
                    print(Fore.RED + "❌ Lỗi: Dữ liệu API 10 ván không hợp lệ. Kiểm tra UID/Key."); continue
                lich_su_chi_tiet = data_10["data"]
                
                # Lấy thống kê 100 ván
            ...
            latest_issue = lich_su_chi_tiet[0]
            vong_choi_moi = latest_issue["issue_id"]
            ...
            print(Fore.LIGHTCYAN_EX + f"\nVòng #{vong_choi_moi} đã kết thúc. Sát thủ ở: {ten_phong_vong_truoc}")
            vong_choi_da_xu_ly = vong_choi_moi

            # 2. XỬ LÝ KẾT QUẢ VÁN TRƯỚC VÀ TÍNH TIỀN CƯỢC MỚI
            if phong_da_cuoc is not None:
                ...
                if thang:
                    ...
                    print(Fore.LIGHTMAGENTA_EX + f"🔥 Chuỗi thắng liên tiếp: {chuoi_thang} ván. Reset tiền cược.")
                else:
                    print(Fore.RED + f"❌ THUA! (-{tien_da_cuoc} BUILD)"); chuoi_thang = 0
                    if number_cuoc == 1: current_bet_amount *= multiplier_1; number_cuoc = 2; print(Fore.YELLOW + f"💰 Gấp cược lần 1 (x{multiplier_1}): {int(current_bet_amount)} BUILD")
                    elif number_cuoc == 2: current_bet_amount *= multiplier_2; number_cuoc = 3; print(Fore.YELLOW + f"💰 Gấp cược lần 2 (x{multiplier_2}): {int(current_bet_amount)} BUILD")
                    elif number_cuoc == 3: current_bet_amount *= multiplier_3; number_cuoc = 4; print(Fore.YELLOW + f"💰 Gấp cược lần 3 (x{multiplier_3}): {int(current_bet_amount)} BUILD")
                    else: current_bet_amount = cuoc_ban_dau; number_cuoc = 1; print(Fore.RED + "🚫 Đã đạt gấp cược tối đa! Reset về cược ban đầu.")
                
                tong_loi_lo()
                if not tool_running: break
            
            # 3. GỬI DỮ LIỆU TỔNG HỢP CHO AI VÀ ĐẶT CƯỢC
            lich_su_str_cho_ai = "\n".join([f"Ván #{d['issue_id']}: Sát thủ ở {reverse_room_mapping.get(d['killed_room_id'], 'N/A')}" for d in lich_su_chi_tiet])
            
            # Chuyển key của thống kê từ ID (string) sang tên phòng cho AI dễ đọc
            ...
            if predicted_room_name and predicted_room_name in room_mapping:
                room_id_to_bet = room_mapping[predicted_room_name]
                dat_cuoc(room_id_to_bet, current_bet_amount)
            else:
                print(Fore.RED + "AI không đưa ra dự đoán hợp lệ. Bỏ qua ván này."); phong_da_cuoc = None

    except KeyboardInterrupt: print(Fore.YELLOW + "\n🛑 Tool đã dừng bởi người dùng (Ctrl+C)")
    finally: print(Fore.CYAN + "Cảm ơn bạn đã sử dụng tool và những góp ý giá trị!")